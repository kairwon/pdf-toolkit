...
import { ArrowRight, FileText, GripVertical, Combine, Split, Layers, Image, FileDown, Droplets, DropletOff, FileType } from 'lucide-react'

const defaultTools = [
  {
    path: '/merge',
    title: 'Merge PDF',
    desc: 'Combine multiple PDFs into one. Preview pages and pick exactly which ones to include.',
    icon: Combine,
  },
  {
    path: '/split',
    title: 'Split PDF',
    desc: 'Extract specific pages from a PDF or split it into two separate files.',
    icon: Split,
  },
  {
    path: '/compress',
    title: 'Compress PDF',
    desc: 'Reduce PDF file size losslessly — text stays selectable.',
    icon: FileDown,
  },
  {
    path: '/to-word',
    title: 'PDF to Word',
    desc: 'Convert PDF to Word — OCR applied automatically to scanned pages.',
    icon: FileType,
  },
  {
    path: '/watermark',
  ...
  },
  {
    ...
    desc: 'Strip overlay watermarks and cover common watermark regions.',
    icon: DropletOff,
  },
  {
    path: '/manage',
    title: 'Manage Pages',
    desc: 'Delete, rotate, or extract pages from your PDF with a visual preview.',
    icon: Layers,
  },
  {
    path: '/to-image',
    title: 'PDF to Image',
    desc: 'Convert PDF pages to PNG or JPEG images. Download individually or as ZIP.',
    icon: Image,
  },
]
  ...
  return (
    <div>
      ...
      <div className="text-center pt-12 pb-10">
        <div className="inline-flex items-center gap-2 bg-jade/10 dark:bg-jade-dark/25 text-jade dark:text-jade-light text-xs font-medium px-3.5 py-1.5 rounded-full mb-5 tracking-wide">
          <FileText size={13} />
        ...
        <Droppable droppableId="tools" direction="horizontal" type="TOOL">
          {(provided, snapshot) => (
            <div
              ...
              className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2.5 mb-12"
            >
              {tools.map((t, index) => (
      ...
      <div className="text-center mb-8">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {futureTools.map((name) => (
            <span key={name} className="text-sm text-gray-400 dark:text-gray-500 bg-white dark:bg-[#1a1a30] px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700/50">{name}</span>
          ))}
        </div>
      </div>
    </div>
