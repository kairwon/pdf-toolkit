...
const pages = {
  ...
  '/compare-pdf': ['Compare PDF Files and Find Changed Pages | Lab of PDF', 'Scan two PDF files locally, find every changed page, inspect highlighted differences, choose detection sensitivity, and download a comparison report.', 'Compare PDF versions'],
  ...
  '/images-to-pdf': ['Convert JPG, PNG or WebP Images to PDF Privately | Lab of PDF', 'Turn JPG, PNG or WebP images into one ordered PDF. Preview page size, orientation and margins before creating it locally without uploads.', 'Images to PDF'],
  ...
  '/word-to-pdf': ['Word to PDF Converter with Preview | Lab of PDF', 'Convert DOC or DOCX to PDF automatically, preview every finished page, and download a searchable PDF with the document layout preserved.', 'Word to PDF'],
  ...
  '/guides/reduce-scanned-pdf-file-size': ['How to Reduce Scanned PDF Size Without Losing Readability', 'Learn why scanned documents are large and how to reduce scanned PDF MB size while protecting handwriting, signatures and small print.', 'Reduce scanned PDF size without losing readability'],
...
const staticBodies = {
  ...
  '/guides/reduce-scanned-pdf-file-size': `<main class="content-article"><article class="content-body"><h1>How to reduce scanned PDF size without losing readability</h1><p>Scanned pages are images, so colour, resolution, backgrounds and blank margins can make a short file large. Scanning at twice the width and height creates roughly four times as many pixels.</p><h2>Resize a scanned document the useful way</h2><p>Changing paper dimensions alone rarely removes scan pixels. Remove blank pages, crop wide margins, rotate pages, and use grayscale only when colour carries no required information.</p><h2>Protect important details</h2><p>Start with balanced compression. Open the downloaded copy and inspect small print, handwriting, stamps, signatures and reference numbers. Confirm page order and size.</p><h2>Common scanned PDF questions</h2><p>To reduce MB size, set the real target and use the mildest mode that meets it. Balanced and maximum modes create image-based copies, while lossless mode keeps searchable text.</p><p><a href="/compress/scanned">Compress a scanned PDF without uploading it</a> · <a href="/crop-pdf">Crop oversized scan margins</a> · <a href="/ocr-pdf">Make the final scan searchable</a></p></article></main>`,
